...
module.exports = class LayoutController
  ...
  @$inject: ['$scope', 'import', '$modal', '$state', 'storage']
  ...
  constructor: (@scope, @import, @modal, state) -> 
    @_searchPending = false
    @_isExpand = false
  ...
  triggerSearch: =>
    ...
    Dancer.findWhere conditions, (err, dancers) =>
      ...
      if err?
        ...
        modalScope.title = i18n.ttl.search 
        modalScope.message = _.sprintf i18n.err.search, err.message
        modalScope.buttons = [label: i18n.btn.nok]
        return @modal.open
          backdrop: true
          keyboard: true
  ...
  importDancers: =>
    modal = $('<input style="display:none;" type="file" accept="application/json,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet"/>')
    modal.change (evt) =>
      filePath = modal.val()
      modal.remove()
      # modal cancellation
      ...
      @scope.$apply => 
        modalScope = @scope.$new()
        modalScope.title = i18n.ttl.search 
        modalScope.message = i18n.msg.importing
        modal = @modal.open
          backdrop: true
          keyboard: true
          templateUrl: "messagebox.html"
      ...
      @import.fromFile filePath, (err, dancers) =>
        ...
        if err?
          ...
          return @scope.$apply =>
            modal.close()
            modalScope = @scope.$new()
            modalScope.title = i18n.ttl.import 
            modalScope.message = _.sprintf i18n.err.importFailed, err.message
        ...
        Dancer.findAll (err, existing) =>
          ...
          @import.merge existing, dancers, (err, imported) =>
            ...
            @scope.$apply =>
              ...
              modalScope = @scope.$new()
              modalScope.title = i18n.ttl.import 
              modalScope.message = if err? then  _.sprintf(i18n.err.importFailed, err.message) else _.sprintf i18n.msg.importSuccess, imported, dancers.length
              modalScope.buttons = [label: i18n.btn.ok]
              @modal.open(
                backdrop: true
                keyboard: true
  ...
  _chooseDumpLocation: (callback) =>
    ...
    modalScope.message = i18n.msg.dumpData
    modalScope.buttons = [label: i18n.btn.ok]
    @modal.open(
      backdrop: true
      keyboard: true
      ...
      modal = $('<input style="display:none;" type="file" nwsaveas value="dump_dancerm.json" accept="application/json"/>')
      modal.change (evt) =>
        dumpPath = modal.val()
        modal.remove()
        # modal cancellation
